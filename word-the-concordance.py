...
for anEssay in allEssays:
    ...
    for aSentence in thisEssay.getSentences():
        ...
        wordNgram = thisSentence.wordNgrams(2, 'the')
...
with open("results/the-ar-concordance.csv", "w", newline='\n') as f:
    writer = csv.writer(f)
    writer.writerows(arThatList)
    
with open("results/the-nar-concordance.csv", "w", newline='\n') as f:
    writer = csv.writer(f)
